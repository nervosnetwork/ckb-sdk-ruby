...
RSpec.describe CKB::Types::Transaction do
  ...
  context "multiple inputs sign" do
    ...
    it "sign" do
      ...
      expect(signed_tx.to_raw_transaction_h[:witnesses]).to eq(%w[
    ...
    let(:tx_to_sign_hash_use_data_hash) do
      { version: "0x0",
        ...
        outputs_data: %w[0x],
        witnesses: [
          CKB::Types::Witness.new(lock: "0x4107bd23eedb9f2a2a749108f6bb9720d745d50f044cc4814bafe189a01fe6fb",
                                  input_type: "0x99caa8d7efdaab11c2bb7e45f4f385d0405f0fa2e8d3ba48496c28a2443e607d", output_type: "0xa6d5e23a77f4d7940aeb88764eebf8146185138641ac43b233e1c9b3daa170fa"), "0x"
        ] }
    end
    ...
    it "sign2" do
      ...
      expect(signed_tx.to_h[:hash]).to eq(tx_hash)
      expect(signed_tx.to_h[:witnesses]).to eq(%w[
  ...
  context "generate tx_hash" do
    ...
    it "should build correct hash when has data1 cell" do
      tx_h = {
        "version": "0x0",
        "cell_deps": [
          {
      ...
      expect(
        tx.compute_hash
      ).to eq "0x9110ca9266f89938f09ae6f93cc914b2c856cc842440d56fda6d16ee62543f5c"
    end
  end
end
